...
// TODO: implement null checks and error handling so that code will not break when api response is
// null.
// TODO: implement a dynamic search, not just getting one client at a time? Do the search first, and
// then get the client if there is a match
@Service
public class ClientApiService {
  ...
  public ClientData getClientData(Long clientId) {
    ...
    try {
      apiResponseEntity =
          restTemplate.exchange(clientUrl, HttpMethod.GET, entity, FineractClientResponse.class);
    } catch (Exception e) {
      throw new IllegalStateException(
          "Unable to fetch client from Fineract for clientId=" + clientId, e);
    }
    FineractClientResponse apiResponse = apiResponseEntity.getBody();
    if (apiResponse == null) {
      throw new IllegalStateException(
          "Fineract returned an empty client payload for clientId=" + clientId);
    }
    ...
    try {
      addressResponseEntity =
          restTemplate.exchange(
              addressUrl, HttpMethod.GET, entity, FineractClientAddressResponse[].class);
    } catch (Exception e) {
      throw new IllegalStateException(
          "Unable to fetch client addresses from Fineract for clientId=" + clientId, e);
    }
    FineractClientAddressResponse[] addressResponse = addressResponseEntity.getBody();

    FineractClientAddressResponse firstAddressResponse =
        addressResponse != null && addressResponse.length > 0 // what if i need multiple addresses
            ? addressResponse[0]
            : null;
