...
public abstract class CirculoDeCreditoResponseToCBCreditReportDataMapper {
  ...
  public CBCreditReportData toCBCreditReportData(CirculoDeCreditoResponse src) {
    ...
    CBCreditReportData.CreditAccount defaultAccount =
        CBCreditReportData.CreditAccount.builder().build();

    // Person
    CBCreditReportData.Person person = mapPerson(src.getPersona());

    // Addresses
    List<CBCreditReportData.Address> addresses =
        safeList(src.getDomicilios()).stream().map(this::mapAddress).collect(Collectors.toList());

    // Employments
    List<CBCreditReportData.Employment> employments =
        safeList(src.getEmpleos()).stream().map(this::mapEmployment).collect(Collectors.toList());

    // Credit Accounts
    List<CBCreditReportData.CreditAccount> accounts =
        safeList(src.getCreditos()).stream()
            .map(this::mapCreditAccount)
            .collect(Collectors.toList());

    // Inquiries
    List<CBCreditReportData.Inquiry> inquiries =
        safeList(src.getConsultas()).stream().map(this::mapInquiry).collect(Collectors.toList());
  ...
  private CBCreditReportData.CreditAccount mapCreditAccount(CirculoDeCreditoResponse.Credito c) {
    ...
    return CBCreditReportData.CreditAccount.builder()
        ...
        .currency(
            nvl(c.getClaveUnidadMonetaria(), guessCurrencyByCode(c.getClaveUnidadMonetaria())))
        .build();
  }
  ...
  private CBCreditReportData.Inquiry mapInquiry(CirculoDeCreditoResponse.Consulta q) {
    ...
    return CBCreditReportData.Inquiry.builder()
        ...
        .currency(
            nvl(q.getClaveUnidadMonetaria(), guessCurrencyByCode(q.getClaveUnidadMonetaria())))
        .build();
  }
  ...
  private String guessCurrency(CirculoDeCreditoResponse src) {
    ...
    if (src.getCreditos() != null && !src.getCreditos().isEmpty()) {
      code =
          src.getCreditos().stream()
              .map(CirculoDeCreditoResponse.Credito::getClaveUnidadMonetaria)
              .filter(Objects::nonNull)
              .findFirst()
              .orElse(null);
    }
    if (code == null && src.getConsultas() != null && !src.getConsultas().isEmpty()) {
      code =
          src.getConsultas().stream()
              .map(CirculoDeCreditoResponse.Consulta::getClaveUnidadMonetaria)
              .filter(Objects::nonNull)
              .findFirst()
              .orElse(null);
    }
  ...
  private String findReportDate(CirculoDeCreditoResponse src) {
    ...
    // Prefer the latest fechaReporte from creditos, otherwise any fechaConsulta from consultas,
    // else empty
    String date = null;
